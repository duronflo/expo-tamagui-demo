...
export default ChangeExchangeRate = () => {
    ...
    const { defaultCurrencyCode, getExchangeRate, addNewExchangeRate } = useCurrencies();
    ...
    useEffect(() => {
        const currExchangeRate = getExchangeRate(defaultCurrencyCode, route.params.code);
        if (currExchangeRate) {
            setExchangeRate(currExchangeRate.toString());
            setError('');
        }
    ...
    }, []); // will only run in mount and unmount
    ...
    return (
        <View style={[styles.container, { flexDirection: "column", alignItems: "center" }]}>
            ...
            <Button
                ...
                onPress={() => {
                    addNewExchangeRate({ code1: code1, code2: code2, rate: parseFloat(exchangeRate) });
                    goBack();
                }
                }
            />
